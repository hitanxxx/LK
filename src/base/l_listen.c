...
static status listen_open( listen_t * listening )
{
	...
	if( ERROR == listening->fd ) {
		err_log( "%s --- socket error, errno [%s]", __func__, strerror(errno) );
		return ERROR;
	}
	if( OK != net_non_blocking( listening->fd ) ) {
		close( listening->fd );
		err_log( "%s --- set non blocking", __func__ );
		return ERROR;
	}
	if( ERROR == setsockopt( listening->fd, SOL_SOCKET, SO_REUSEADDR,
		(const void *)&reuseaddr, sizeof(reuseaddr)) ) {
		close( listening->fd );
		err_log( "%s --- setsockopt reuseaddr", __func__ );
		return ERROR;
	}
	if( conf.reuse_port ) {
		if( listening->reuse_port ) {
			if( ERROR == setsockopt( listening->fd, SOL_SOCKET, SO_REUSEPORT,
				...
				err_log( "%s --- setsockopt reuseport", __func__ );
				close( listening->fd );
				return ERROR;
			}
		}
	...
	if( ERROR == setsockopt( listening->fd, IPPROTO_TCP, TCP_FASTOPEN,
		...
		err_log( "%s --- set fastopen", __func__ );
		return ERROR;
	}
	if( ERROR == setsockopt( listening->fd, IPPROTO_TCP, TCP_NODELAY,
			(const void *) &tcp_nodelay, sizeof(tcp_nodelay)) ) {
		err_log( "%s --- set nodelay", __func__ );
		return ERROR;
	}
	if( OK != bind( listening->fd, (struct sockaddr *)&listening->server_addr,
		sizeof(struct sockaddr) ) ) {
		err_log( "%s --- bind, errno [%s]", __func__, strerror(errno) );
		close( listening->fd );
		return ERROR;
	}
	if( OK != listen( listening->fd, 500 ) ) {
		err_log( "%s -- listen, errno [%s]", __func__, strerror(errno) );
		close( listening->fd );
		return ERROR;
	}
	if( OK != net_non_blocking( listening->fd ) ) {
		err_log( "%s --- non_blocking, errno [%s]", __func__, strerror(errno) );
		close( listening->fd );
		return ERROR;
	}
	return OK;
...
status listen_start( void )
{
	...
	for( i = 0; i < listens->elem_num; i ++ ) {
		...
		if( OK != listen_open( listen_head ) ) {
			err_log("%s --- listen open head", __func__ );
			listen_head->error = 1;
			return ERROR;
		}
		if( listen_head->reuse_port ) {
			...
			for( j = 0; j < process_num; j ++ ) {
				...
				if( OK != listen_open( listen ) ) {
					err_log("%s -- listen open", __func__ );
					listen->error = 1;
					return ERROR;
				}
			}
